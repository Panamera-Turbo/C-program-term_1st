...
#include <string.h>
#include <stdbool.h>
#include "list_point.h"//符号 “ ”让系统只在当前目录下搜索
 ...
 size_t list_create_from_file(listptr l, const char* filename)
{
    ...
    fp = fopen(filename, "rt");//rt: read txt只读不写；wt: write txt只写不读； wr或at  可读可写； 
    if (fp==NULL)
    {
...
void list_save_to_file(listptr l, const char* filename)
{
    ...
    fp = fopen(filename, "wt");//rt: read txt只读不写；wt: write txt只写不读； wr或at  可读可写； 
    if (fp==NULL)
    {
        printf("Can not open file %s", filename);
        exit(1);
